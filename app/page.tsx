...
import MobileHome from "@/features/home/components/mobile-home";
...
export default async function HomePage() {
  ...
  return (
    <>
      {/* ── 1. MOBILE EXPERIENCE (Strictly < md) ── */}
      <div className="block md:hidden">
        <MobileHome properties={featured} />
      </div>

      {/* ── 2. DESKTOP EXPERIENCE (Strictly >= md — 100% Intact and Untouched) ── */}
      <main className="hidden md:block min-h-screen overflow-hidden bg-[#f5f0e8]">
        {/* ── PREMIUM ROUNDED CARD (minimum full screen to push stats strictly below fold) ── */}
        <div className="mx-auto max-w-[1400px] px-2 pt-2 pb-0 sm:px-3 md:px-5 md:pt-5">
          <div className="flex flex-col rounded-[1.25rem] border border-[#e5ddd0] bg-[#faf7f2] shadow-[0_2px_32px_rgba(0,0,0,0.05)] md:min-h-[calc(100vh-20px)] md:rounded-[2.25rem]">
            <div>
              {/* 1. NAVBAR — always the topmost element */}
              <HomepageNavbar />

              {/* 2. SEARCH BAR — centered and scaled down */}
              <div className="flex justify-center px-3 pt-3 pb-1.5 sm:px-6 sm:pt-4 md:px-10 md:pt-5 lg:px-14">
