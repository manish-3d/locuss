...
import { MobilePropertiesView } from "./components/mobile-properties-view";
import { MobilePropertyMap } from "@/components/property/mobile-property-map";
...
export default async function PropertiesPage({ searchParams }: Props) {
  ...
  return (
    <>
      {/* ── 1. MOBILE EXPERIENCE (< md) ── */}
      <div className="block md:hidden">
        {isMapView ? (
          <MobilePropertyMap properties={mapProperties} totalCount={totalCount} />
        ) : (
      ...
      {/* ── 2. DESKTOP EXPERIENCE (>= md — 100% Intact & Untouched) ── */}
      <main className="hidden md:block mx-auto w-full max-w-[1536px] overflow-hidden px-3 py-3 sm:px-5 sm:py-6 lg:px-6">
        {/* Header section — Compact & Proportional */}
        <div className="flex flex-col gap-3 border-b border-[#e5ddd0] pb-3 sm:gap-3 md:flex-row md:items-center md:justify-between sm:pb-4">
          <div>
            ...
            <h1 className="mt-1 font-serif text-[1.4rem] font-bold tracking-tight text-[#1e1b17] sm:text-2xl lg:text-[1.85rem]">
              {listingTypeLabel}
            </h1>
          </div>

          <div className="flex w-full items-center gap-2 sm:w-auto sm:flex-wrap sm:gap-2.5">
            {!isMapView && <SortSelect />}
            <ViewToggle />
          </div>
        </div>

        <div className="mt-3 flex flex-col gap-3.5 lg:mt-5 lg:flex-row lg:items-start lg:gap-4">
          ...
          <div className="flex-1 min-w-0">
            {properties.length > 0 ? (
              isMapView ? (
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-2 sm:gap-2.5 md:grid-cols-3 lg:grid-cols-4">
